...
public class FIllRectangleWithWords {
	private int maxWordLength;
	private WordGroup[] groupList;
	private Trie1 trieList[];

	public FIllRectangleWithWords(String[] list) {
		groupList = WordGroup.createWordGroups(list);
		maxWordLength = groupList.length;
		// Initialize trieList to store Trie1 of groupList[i] at ith position.
		trieList = new Trie1[maxWordLength];
	}
	...
	private Rectangle makeRectangle(int length, int height) {
		...
		if (trieList[height - 1] == null) {
			...
			trieList[height - 1] = new Trie1(words);
		}
	 ...
	 * with length l as the first row. We then check the Trie1 of words of
	 * length h to see if each partial column is a prefix of a word with length
	 * h. If so we branch recursively and check the next word till we've formed
	 * a complete rectangle. When we have a complete rectangle check if every
...
class Rectangle {
	...
	public boolean isPartialOK(int l, Trie1 Trie1) {
		if (height == 0) {
			return true;
		}
		for (int i = 0; i < l; i++) {
...
class Trie1 {
	// The root of this Trie1.
	private TrieNode1 root;

	/*
	 * Takes a list of strings as an argument, and constructs a Trie1 that
	 * stores these strings.
	 */
	public Trie1(ArrayList<String> list) {
		root = new TrieNode1();
	...
	/*
	 * Checks whether this Trie1 contains a string with the prefix passed in as
	 * argument.
	 */
...
class TrieNode1 {
	...
	public void addWord(String word) {
		...
		TrieNode1 child = getChild(firstChar);
		if (child == null) {
			child = new TrieNode1(firstChar);
