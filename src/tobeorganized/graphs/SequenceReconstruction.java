 ...
 *
 * <p>Unique reconstruction means that we need to find if originalSeq is the only sequence such that
...
public class SequenceReconstruction {
  public static boolean canConstruct(int[] originalSeq, int[][] sequences) {
    ...
    // sequence
    if (inDegree.size() != originalSeq.length) return false;
  ...
  public static void main(String[] args) {
    boolean result =
        SequenceReconstruction.canConstruct(
            new int[] {1, 2, 3, 4},
    ...
    result =
        SequenceReconstruction.canConstruct(
            new int[] {3, 1, 4, 2, 5}, new int[][] {new int[] {3, 1, 5}, new int[] {1, 4, 2, 5}});
    System.out.println("Can we uniquely construct the sequence: " + result);
  }
}
