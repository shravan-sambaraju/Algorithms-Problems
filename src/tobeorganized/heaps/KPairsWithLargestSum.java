...
public class KPairsWithLargestSum {
    ...
    for (int i = 0; i < nums1.length && i < k; i++) {
      for (int j = 0; j < nums2.length && j < k; j++) {
        if (minHeap.size() < k) {
          ...
          // element of
          // the heap, we can 'break' here. Since the arrays are sorted in the descending order,
          // we'll not be
          // able to find a pair with a higher sum moving forward.
          if (nums1[i] + nums2[j] < minHeap.peek()[0] + minHeap.peek()[1]) {
            break;
          } else { // else: we have a pair with a larger sum, remove top and insert this pair in the
            // heap
            minHeap.poll();
            minHeap.add(new int[] {nums1[i], nums2[j]});
          }
  ...
  public static void main(String[] args) {
    ...
    for (int[] pair : result) System.out.print("[" + pair[0] + ", " + pair[1] + "] ");
  }
}
