...
class PalindromicPartitioning {
  ...
  private int findMPPCutsRecursive(
      Integer dp[][], Boolean dpIsPalindrome[][], String st, int startIndex, int endIndex) {

    if (startIndex >= endIndex || isPalindrome(dpIsPalindrome, st, startIndex, endIndex)) return 0;
    ...
    if (dp[startIndex][endIndex] == null) {
      ...
      for (int i = startIndex; i <= endIndex; i++) {
        if (isPalindrome(dpIsPalindrome, st, startIndex, i)) {
          ...
          minimumCuts =
              Math.min(
                  minimumCuts, 1 + findMPPCutsRecursive(dp, dpIsPalindrome, st, i + 1, endIndex));
        }
      }
