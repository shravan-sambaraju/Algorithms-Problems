...
public class MinStepsToReachEndArrayWithFee {

  public static void main(String[] args) {
    MinStepsToReachEndArrayWithFee sc = new MinStepsToReachEndArrayWithFee();
    int[] fee = {1, 2, 5, 2, 1, 2};
  ...
  public int findMinFeeBottomUp(int[] fee) {
    ...
    for (int i = 3; i < fee.length; i++)
      dp[i + 1] =
          Math.min(fee[i] + dp[i], Math.min(fee[i - 1] + dp[i - 1], fee[i - 2] + dp[i - 2]));
